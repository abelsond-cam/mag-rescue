...
The build script runs on linux-64 only — Kleborate's bioconda package is linux-only. Run on HPC:

```bash
ssh login.hpc.cam.ac.uk
cd ~/workspace/mag-rescue
git pull
pixi run -e dev python -m mag_rescue.pp.build_ariba_ref --kleb-virulence --threads 4 [--force]
```

The script:
1. locates the pixi-installed `kleborate` package,
2. copies the 39 allele FASTAs (5 virulence loci × 6 modules — yes, 6: `rmp` is split across `klebsiella__rmst` and `klebsiella__rmpa2` upstream) plus their `profiles.tsv` files into `refs/kleb_virulence/inputs/`,
3. emits `metadata.tsv` (one row per allele, `gene=1, variant_only=0, variant=., variant_id=., free_text=<cluster>:<gene>` — six tab-separated columns; ARIBA silently rejects rows with the wrong column count),
4. writes `manifest.json`,
5. runs `ariba prepareref` with default CD-HIT settings (cdhit_min_id 0.9). Most of the 39 input genes collapse to one cluster, but ~14 form an additional outlier sub-cluster for divergent allele lineages.

Then commit the refreshed `inputs/`, `metadata.tsv`, `manifest.json` from HPC and push.

### Refreshing when Kleborate updates

The Kleborate version is pinned by `kleborate = ">=3.2"` in `pixi.toml`'s `[target.linux-64.dependencies]`. To pull newer alleles:

```bash
# locally or on HPC
pixi update kleborate     # bump within the >=3.2 floor
git add pixi.lock && git commit -m "chore: bump kleborate"
git push

# on HPC, rebuild the ref DB
git pull && pixi install -e dev
pixi run -e dev python -m mag_rescue.pp.build_ariba_ref --kleb-virulence --force
